...
export default function ATKPurchaseBook() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/accounting/atk-purchase-book?from=${dateFrom}&to=${dateTo}`, { credentials: 'include' });
  ...
  const exportExcel = async () => {
    ...
    const wsData = [
      ['LIBRI I BLERJEVE - ATK'],
      [`Periudha: ${moment(dateFrom).format('DD/MM/YYYY')} - ${moment(dateTo).format('DD/MM/YYYY')}`],
      [],
      ['Nr.', 'Përshkrimi', 'Data', 'Furnitori', 'Kategoria', 'Vlera pa TVSH', 'TVSH', 'Vlera me TVSH'],
      ...data.map((row, i) => [
        i + 1,
        row.description || '',
    ...
    ws['!cols'] = [{ wch: 5 }, { wch: 25 }, { wch: 12 }, { wch: 25 }, { wch: 15 }, { wch: 15 }, { wch: 12 }, { wch: 15 }];
    const wb = utils.book_new();
    utils.book_append_sheet(wb, ws, 'Libri i Blerjeve');
    writeFile(wb, `libri-blerjeve-atk-${dateFrom}-${dateTo}.xlsx`);
  };
  ...
  const exportPDF = async () => {
    ...
    doc.text('LIBRI I BLERJEVE - ATK', margin, 15);
    doc.setFontSize(8);
    ...
    doc.text('Përshkrimi', margin + 15, y);
    doc.text('Data', margin + 80, y);
    doc.text('Furnitori', margin + 110, y);
    doc.text('Kategoria', margin + 170, y);
    doc.text('Pa TVSH', margin + 210, y);
    doc.text('TVSH', margin + 240, y);
    ...
    data.forEach((row, i) => {
      ...
      doc.text((row.description || '').slice(0, 30), margin + 15, y);
      doc.text(row.expense_date ? moment(row.expense_date).format('DD/MM/YYYY') : '', margin + 80, y);
      doc.text((row.supplier_name || '').slice(0, 25), margin + 110, y);
      doc.text((row.category_name || '').slice(0, 20), margin + 170, y);
      doc.text(parseFloat(row.subtotal || 0).toFixed(2), margin + 210, y);
      doc.text(parseFloat(row.tax_amount || 0).toFixed(2), margin + 240, y);
    ...
    doc.setTextColor(255, 255, 255);
    doc.setFont('helvetica', 'bold');
    doc.text('TOTALI', margin + 110, y + 7);
    doc.text(totalSubtotal.toFixed(2), margin + 210, y + 7);
    doc.text(totalTax.toFixed(2), margin + 240, y + 7);
    doc.text(totalAmount.toFixed(2), W - margin - 2, y + 7, { align: 'right' });

    doc.save(`libri-blerjeve-atk-${dateFrom}-${dateTo}.pdf`);
  };
  ...
  return (
    <div className="p-6 lg:p-10 space-y-8">
      <div>
        ...
        <h1 className="text-3xl font-bold tracking-tight" data-testid="text-page-title">Libri i Blerjeve</h1>
        <p className="text-sm text-muted-foreground mt-1">ATK Purchase Book - Regjistri i blerjeve sipas formatit fiskal</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <Card className="p-5">
          ...
          <p className="text-2xl font-bold mt-1" data-testid="text-expense-count">{data.length}</p>
        </Card>
        <Card className="p-5">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/20">
                ...
                <th className="text-left py-3 px-4 font-semibold text-muted-foreground">Furnitori</th>
            ...
            <tbody>
              {data.length === 0 ? (
                <tr><td colSpan="8" className="text-center py-12 text-muted-foreground">Nuk ka shpenzime për këtë periudhë</td></tr>
              ) : data.map((row, i) => (
                <tr key={row.id} className={`${i % 2 === 0 ? 'bg-muted/10' : ''}`} data-testid={`row-purchase-${row.id}`}>
                  <td className="py-2.5 px-4 text-muted-foreground">{i + 1}</td>
                  <td className="py-2.5 px-4 max-w-[200px] truncate">{row.description || '-'}</td>
                  <td className="py-2.5 px-4">{row.expense_date ? moment(row.expense_date).format('DD/MM/YYYY') : '-'}</td>
                  <td className="py-2.5 px-4">{row.supplier_name || '-'}</td>
                  <td className="py-2.5 px-4 text-muted-foreground">{row.category_name || '-'}</td>
