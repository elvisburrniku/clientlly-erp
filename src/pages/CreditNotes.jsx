...
export default function CreditNotes() {
  ...
  const loadData = async () => {
    const [user, data, invs] = await Promise.all([
      base44.auth.me(),
      base44.entities.CreditNote.list("-created_date", 100),
  ...
  const fillFromInvoice = (invoiceId) => {
  ...
  const handleCreate = async () => {
    ...
    await base44.entities.CreditNote.create({
      tenant_id: tenantId,
    ...
    setForm(emptyForm());
    setSubmitting(false);
    toast.success("Nota kreditore u krijua");
    loadData();
  };
  ...
  const handleUpdate = async () => {
    ...
    setSubmitting(true);
    const { subtotal, vat_amount, amount } = calcTotals(form.items);
    await base44.entities.CreditNote.update(editNote.id, {
      client_name: form.client_name,
      client_email: form.client_email,
      client_phone: form.client_phone,
    ...
    setForm(emptyForm());
    setSubmitting(false);
    toast.success("Nota kreditore u përditësua");
    loadData();
  };

  const handleApply = async (note) => {
    if (note.invoice_id) {
      const inv = invoices.find(i => i.id === note.invoice_id);
      if (inv) {
        const newAmount = Math.max(0, (inv.amount || 0) - (note.amount || 0));
        await base44.entities.Invoice.update(inv.id, { amount: newAmount });
      }
    }
    await base44.entities.CreditNote.update(note.id, { status: "applied" });
    toast.success("Nota kreditore u aplikua në faturë");
    loadData();
  };

  const handleDelete = async (note) => {
    if (!window.confirm(`Fshi notën kreditore ${note.credit_note_number}?`)) return;
    await base44.entities.CreditNote.delete(note.id);
    toast.success("Nota kreditore u fshi");
    loadData();
  };
  ...
  const generatePDF = (note) => {
    ...
    doc.text("NOTË KREDITORE", margin, 13);
    doc.setFontSize(10); doc.setFont("helvetica", "normal");
    doc.text(note.credit_note_number || "", margin, 22);
    ...
    doc.text(note.client_name || "", margin + 20, y);
    y += 7;
    if (note.invoice_number) {
      doc.setFont("helvetica", "bold");
      doc.text("Ref. Faturë:", margin, y);
      doc.setFont("helvetica", "normal");
      doc.text(note.invoice_number, margin + 28, y);
      y += 7;
    }
    if (note.reason) {
      doc.setFont("helvetica", "bold");
    ...
    doc.save(`nota_kreditore_${note.credit_note_number || "draft"}.pdf`);
  };
  ...
  const filtered = notes.filter(n => {
    ...
    return n.client_name?.toLowerCase().includes(q) || n.credit_note_number?.toLowerCase().includes(q);
  });
    ...
    <div className="p-6 lg:p-10 space-y-8">
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold tracking-tight">Nota Kreditore</h1>
        </div>
        <Button onClick={() => { setForm(emptyForm()); setEditNote(null); setDialogOpen(true); }} className="gap-2 rounded-xl" data-testid="button-new-credit-note">
      ...
      <div className="relative max-w-sm">
        ...
        <Input placeholder="Kërko..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 rounded-xl" data-testid="input-search-credit-notes" />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-border/60 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={7} className="text-center text-muted-foreground py-12">Asnjë notë kreditore</td></tr>
            ) : filtered.map((note) => {
              const sc = statusConfig[note.status] || statusConfig.draft;
              return (
                <tr key={note.id} className="border-b border-border last:border-0 hover:bg-muted/20" data-testid={`row-credit-note-${note.id}`}>
                  <td className="px-4 py-3 font-medium">{note.credit_note_number}</td>
                  <td className="px-4 py-3">{note.client_name}</td>
                  <td className="px-4 py-3 text-muted-foreground">{note.invoice_number || "—"}</td>
                  <td className="px-4 py-3 text-right font-semibold text-red-600">€{(note.amount || 0).toFixed(2)}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" className="h-8 w-8" data-testid={`button-actions-credit-note-${note.id}`}><MoreHorizontal className="w-4 h-4" /></Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end">
                        ...
                        {note.status === "draft" && note.invoice_id && (
                          <DropdownMenuItem onClick={() => handleApply(note)}><FileText className="w-4 h-4 mr-2" /> Apliko në Faturë</DropdownMenuItem>
                        )}
      ...
      <Dialog open={dialogOpen || !!editNote} onOpenChange={(open) => { if (!open) { setDialogOpen(false); setEditNote(null); setForm(emptyForm()); } }}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editNote ? "Ndrysho Notën Kreditore" : "Notë Kreditore e Re"}</DialogTitle>
            <DialogDescription>Plotësoni detajet e notës kreditore</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            {!editNote && (
              <div>
                <Label>Referenca e Faturës (opsionale)</Label>
                <Select value={form.invoice_id} onValueChange={(v) => fillFromInvoice(v)}>
                  <SelectTrigger><SelectValue placeholder="Zgjidh faturën..." /></SelectTrigger>
                  <SelectContent>
                    {invoices.map(inv => (
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input value={form.client_name} onChange={e => setForm({ ...form, client_name: e.target.value })} data-testid="input-credit-note-client" />
              </div>
              <div>
                <Label>Email</Label>
            ...
            <div>
              <Label>Arsyeja</Label>
              <Textarea value={form.reason} onChange={e => setForm({ ...form, reason: e.target.value })} placeholder="Arsyeja e notës kreditore..." />
            </div>
            <div>
            ...
            <div className="flex justify-end gap-4 text-sm">
              ...
              <span>Total: <b className="text-red-600">€{formTotals.amount.toFixed(2)}</b></span>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setDialogOpen(false); setEditNote(null); setForm(emptyForm()); }}>Anulo</Button>
            <Button onClick={editNote ? handleUpdate : handleCreate} disabled={submitting} data-testid="button-save-credit-note">
